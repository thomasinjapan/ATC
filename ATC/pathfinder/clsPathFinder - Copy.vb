        ...
        While nextCardPointer <= Me.queueWaiting.Count - 1
            ...
            If Not nextCard.node.objectID = endNode.objectID Then
                ...
                For C1 As Long = 0 To nextCard.node.taxiWays.GetUpperBound(0)
                    'if we already took that path, don't check it again
                    If Not nextCard.pathvisited(C1) Then
                        ...
                        If angleDelta <= maxAngle Then
                            ...
                            Next
                            Dim proposedNewCard As New structInfoCard With {.node = nextCard.node.taxiWays(C1).oppositeTaxiWayPoint(nextCard.node), .knownShortestDistance = newDistance, .previousNode = nextCard.node, .previousTaxiWay = nextCard.node.taxiWays(C1), .previousDirection = newAngle, .pathvisited = propsedpathsvisited, .cameFromIndex = nextCardPointer, .name = nextCard.node.taxiWays(C1).oppositeTaxiWayPoint(nextCard.node).objectID}
                        ...
                        End If
                    Else
                        'this path was already visited - don't look again
                    End If
