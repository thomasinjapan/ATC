...
Public Class clsDijkstra
    ...
    Public Sub New(ByRef startNode As clsConnectionPoint, ByRef endNode As clsConnectionPoint, Optional startAngle As Double = 0, Optional ByVal maxAngle As Double = 360, Optional ByVal runwayPunsihment As Double = 0, Optional ByVal preferences As List(Of String) = Nothing)
        ...
        '!!!
        Dim startpathvisited(startCard.node.taxiWays.GetUpperBound(0)) As Boolean
        For C1 As Long = 0 To startpathvisited.GetUpperBound(0)
            startpathvisited(C1) = False
        Next
        startCard.pathvisited = startpathvisited
            ...
            If Not nextCard.node.objectID = endNode.objectID Then
                ...
                For C1 As Long = 0 To nextCard.node.taxiWays.GetUpperBound(0)
                    ...
                    If angleDelta <= maxAngle Then
                        ...
                        '!!!
                        Dim propsedpathsvisited(nextCard.node.taxiWays(C1).oppositeTaxiWayPoint(nextCard.node).taxiWays.GetUpperBound(0)) As Boolean
                        For C2 As Long = 0 To propsedpathsvisited.GetUpperBound(0)
                            propsedpathsvisited(C2) = False
                        Next
                        startCard.pathvisited = startpathvisited
                        Dim proposedNewCard As New structInfoCard With {.node = nextCard.node.taxiWays(C1).oppositeTaxiWayPoint(nextCard.node), .knownShortestDistance = newDistance, .previousNode = nextCard.node, .previousTaxiWay = nextCard.node.taxiWays(C1), .previousDirection = newAngle, .pathvisited = propsedpathsvisited}
                ...
                '!!!
                'if at least one path is not visited yet, make a copy of this card with long distance and keep it here
                Dim missedPath As Boolean = False
                For C1 As Long = 0 To nextCard.pathvisited.GetUpperBound(0)
                    If nextCard.pathvisited(C1) = False Then
                        missedPath = True
                        Exit For
                    End If
                Next
                ...
                '!!!
                If missedPath Then
                    Dim copycard As New structInfoCard With {.knownShortestDistance = nextCard.knownShortestDistance + 1000,
                                                             .pathvisited = nextCard.pathvisited,
                                                             .node = nextCard.node,
                                                             .previousDirection = nextCard.previousDirection,
                                                             .previousNode = nextCard.previousNode,
                                                             .previousTaxiWay = nextCard.previousTaxiWay}
                    queueWaiting.Add(copycard)
                End If
